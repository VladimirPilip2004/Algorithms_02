...
public class Main {
    // Возвращаем левого потомка `A[i]`
    private static int LEFT(int i) {
        return (2 * i + 1);
    }

    // Возвращаем правого потомка `A[i]`
    private static int RIGHT(int i) {
        return (2 * i + 2);
    }

    // Функция замены двух индексов в массиве
    ...
    private static void heapify(int[] A, int i, int size) {
        ...
        // Обмен местами с потомком, имеющим большее значение и вызовите heapify-down для дочернего элемента
        if (largest != i) {
            swap(A, i, largest);
